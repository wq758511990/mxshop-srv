...
func (s *GoodsServer) GoodsList1(ctx context.Context, req *proto.GoodsFilterRequest) (*proto.GoodsListResponse, error) {
	...
	if req.TopCategory > 0 {
		...
		if category.Level == 1 {
			subQuery = fmt.Sprintf("select id from category where parent_category_id in (select id from category where parent_category_id=#{req.TopCategory})")
		} else if category.Level == 2 {
			subQuery = fmt.Sprintf("select id from category where parent_category_id=#{req.TopCategory}")
		} else if category.Level == 3 {
			subQuery = fmt.Sprintf("select id from category where id=#{req.TopCategory}")
		}
		...
		for _, re := range results {
			categoryIds = append(categoryIds, re)
		}
...
func (s *GoodsServer) GetGoodsDetail(ctx context.Context, req *proto.GoodInfoRequest) (*proto.GoodsInfoResponse, error) {
	...
	if result := global.DB.Preload("Category").Preload("Brand").First(&goods, req.Id); result.RowsAffected == 0 {
		return nil, status.Errorf(codes.NotFound, "商品不存在")
	}
